...
public class Parser {
	...
	private static final HashMap<String, TokenType> tokenMap = new HashMap<String, TokenType>() {
		private static final long serialVersionUID = 1L;
		{
			for (TokenType each : TokenType.values()) {
				String name = each.name().toUpperCase();
				TokenType t = each;
				put(name, t);
			}
		}
	...
	private List<Entry<TokenType, String>> generateToken(String line) throws LexiconException {
		...
		List<Entry<TokenType, Pattern>> patterns = makeTokenPatterns("resources.languages/Syntax");
		...
		while (st.hasMoreTokens()) {
			...
			if (!matchFlag)
				throw new LexiconException("Ilegal Input: Cannot find a matching token!");
		}
		return result;
	}
